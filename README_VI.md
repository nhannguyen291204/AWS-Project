...
2. Tạo file cấu hình từ file mẫu:
   ...
   cp .env.example .env
   ```
   _(Nếu dùng Windows CMD: `copy .env.example .env`)_

### 🔹 Bước 2: Điền thông tin AWS vào file `.env`

Mở file `.env` vừa tạo và điền các thông tin sau:

1. Vào trang **AWS Academy Learner Lab** ➡️ bấm **AWS Details** ➡️ bấm chữ **Show** ở mục _AWS CLI credentials_.
...
_(Script sẽ tự động cài Node.js, Nginx, cài `npm install` và chạy service ngầm)_
...
_(Hiển thị `active (running)` màu xanh là hoàn tất 100%)_
